 ...
 * Run this once to initialize/update the database schema.
...
async function initSchema() {
  ...
  if (currentVersion < 1) {
    ...
    console.log('Applied migration version 1: Initial schema');
  }
...
async function main() {
  try {
    ...
    const version = await initSchema();
    console.log(`Migration completed successfully. Current schema version: ${version}`);
    process.exit(0);
  } catch (error) {
