...
import { requireValidUUID, validateAndNormalizeCharacterSheet } from '@/lib/validation';
...
export async function POST(req: Request, ctx: { params: Promise<{ roomId: string }> }) {
  ...
  try {
    ...
    // Validate and normalize sheet data (stricter schema checks + useful errors)
    const { sheet: sheet_json, issues } = validateAndNormalizeCharacterSheet(body.sheet, { level });
    if (issues.length > 0) {
      const err = Errors.badRequest(`Invalid character sheet (${issues.length} issue${issues.length === 1 ? '' : 's'})`);
      return NextResponse.json(
        {
          error: err.message,
          code: 'INVALID_CHARACTER_SHEET',
          requestId,
          issues,
        },
