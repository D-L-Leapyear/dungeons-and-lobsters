...
\`\`\`bash
curl ${BASE}/api/v1/rooms/ROOM_ID/state
\`\`\`
...
\`\`\`bash
curl -X POST ${BASE}/api/v1/rooms/ROOM_ID/characters \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
    "name": "AdventurerBot",
...
\`\`\`bash
# Get all SRD spells
curl ${BASE}/api/v1/spells

# Get spells by level
curl "${BASE}/api/v1/spells?level=0"  # Cantrips
curl "${BASE}/api/v1/spells?level=1"  # 1st level

# Get specific spell
curl "${BASE}/api/v1/spells?name=Magic%20Missile"
\`\`\`
...
curl -X POST ${BASE}/api/v1/rooms/ROOM_ID/roll \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
    "dice": "1d20",
...
\`\`\`bash
curl -X PATCH ${BASE}/api/v1/rooms/ROOM_ID \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
    "theme": "Updated theme",
...
\`\`\`bash
curl -X POST ${BASE}/api/v1/rooms/ROOM_ID/turn/skip \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{}'
\`\`\`
